...
terrain(dem, v = 'slope', neighbors = 8, unit = 'degrees', 
               filename = paste0(here(), '/data/processed/UFO_slope.tif')
               )
...
```{r Create Geomorphons and Hydrologic Basins of Field Office}


Sys.time()
wbt_geomorphons('/home/steppe/AIM_field/data/raw/UFO_dem_10m.tif', 
                output = p, search = 50, forms = T)
Sys.time()


input <- sample_dem_data()
wbt_geomorphons(input, output = p, search = 50, forms = T)
f <- rast(p)
plot(f)
```
...
ggplot(counties) +
  ...
  geom_sf(data = cnty_rds_buf, alpha = 0.7) +
  theme_bw()

cnty_rds_buf <- st_buffer(cnty_rds, 4828) %>% 
  st_union()
...
### Determine accuracy of point drops on BLM Land. 

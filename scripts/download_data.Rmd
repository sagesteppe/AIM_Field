...
```{r worked examples}
park2point <- np_linestrings %>% 
  st_cast('MULTIPOINT') %>% 
  st_as_sf() %>% 
  st_cast('POINT') %>% 
  mutate(location = rep(c('point', 'parking'), times = length(np_linestrings)), .before = 1) %>% 
  mutate(plot_id = rep(pts$plot_id, each = 2), .before = 1) %>% 
  rename('geometry' = x)

dem <- raster::raster(paste0(here(), '/data/processed/', "UFO_dem_10_smooth.tif"))
...
dem <- terra:project(dem, 29612)

perspective_routes <- function(target_pts, roads, dem, search_dist, public_lands, utm){
  ...
  # dem: a digital elevation model, in meters (we recommend terra for projecting)
  ...
  # utm: a local utm coordinate system for use. 
  
  # REDUCE EXTENT OF ANALYSIS TO STUDY AREAS 
  
  pts <- st_transform(target_pts, utm)
  search_window <- st_buffer(pts, search_dist)
  
  roads <- st_transform(roads, utm)
  road_subset <- st_intersection(search_window, roads)
  road_susbet <- st_segmentize(road_subset, dfMaxLength = 400)
  
  # identify the 5 nearest road segments
  
  nearest_segment_1 <- st_nearest_feature(target_pts, roads)
  closest_segment_1 <- cnty_rds[nearest_segment_1,]
  distances <- st_distance(pts, cnty_rds[res,], by_element = TRUE)
  np_linestrings <- st_nearest_points(pts, closest_rds, pairwise = TRUE)
